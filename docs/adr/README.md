...
| [0014](0014-the-evidence-pull-request-merges-itself.md) | the evidence pull request is merged by the job that opened it | `--auto`, measured impossible here, and a required-check ruleset, measured worse. Amended: the merge waits for `fast` and `databricks evidence`, dispatched on the evidence branch |
