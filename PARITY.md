...
| bundles (Declarative Automation Bundles, formerly Asset Bundles) | n/a | supported, from outside the workspace with a PAT - **this repository deploys one**, and the runs it produced are committed under `evidence/databricks/`; see `docs/databricks-run.md` | Free Edition restricts outbound traffic, so deploying *from inside* the workspace is unreliable; deploy from outside |
