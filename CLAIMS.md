...
|---|---|---|---|---|
| `SG-00` what this repository contains, counted | PASS | 431/431 (95% CI 99.1%-100.0%) | oss-local | local run, not reproduced in CI, 5adbbe403 |
| `SG-01` two implementations agree on the close | PASS | 15/15 (95% CI 79.6%-100.0%) | oss-local | local run, not reproduced in CI, 5adbbe403 |
| `SG-02` re-delivery under a new path is a no-op | PASS | 3/3 (95% CI 43.9%-100.0%) | oss-local | local run, not reproduced in CI, 5adbbe403 |
| `SG-03` mutation campaign | PASS | 67/67 (95% CI 94.6%-100.0%) | oss-local | local run, not reproduced in CI, 5adbbe403 |
| `SG-04` a closed month moves after it is closed | PASS | 2/2 (95% CI 34.2%-100.0%) | oss-local | local run, not reproduced in CI, 5adbbe403 |
| `SG-05` dimension and conservation invariants hold without an oracle | PASS | 3/3 (95% CI 43.9%-100.0%) | oss-local | local run, not reproduced in CI, 5adbbe403 |
| `SG-06` the evidence chain verifies and every seed derives from its commit | PASS | 171/171 (95% CI 97.8%-100.0%) | oss-local | local run, not reproduced in CI, 5adbbe403 |
| `SG-07` the silver writer survives a crash at each of its structural points | PASS | 20/20 (95% CI 83.9%-100.0%) | oss-local | local run, not reproduced in CI, 5adbbe403 |
| `SG-08` no direct identifier reaches gold, and a purge really purges | PASS | 6/6 (95% CI 61.0%-100.0%) | oss-local | local run, not reproduced in CI, 5adbbe403 |
| `SG-09` what layout costs, in files and bytes | PASS | 5/5 (95% CI 56.6%-100.0%) | oss-local | local run, not reproduced in CI, 5adbbe403 |
...
<!--sg:SG-04.artifact.worst_move_pct-->5.5515<!--/sg-->%.
...
beside it as
<!--sg:SG-04.artifact.worst_move_pct_including_boundary_fixtures-->5.4195<!--/sg-->%, so the size
...
after it and is therefore not in this number. Result: <!--sg:SG-06.rate-->171/171 (95% CI 97.8%-100.0%)<!--/sg-->.
...
must be read by <!--sg:SG-09.artifact.share_read_reduction_pct-->76.97<!--/sg-->%. The headline is
