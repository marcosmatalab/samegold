...
Every adversarial round this repository has been through, and until this file existed what each one found lived in a commit message. A
...
| **The number is the finding** | Three was a reviewer being unlucky with the first page. Sixty-seven is a property of the repository: the anchored figures were checked on every push for twenty rounds while the sentences around them were checked by nobody, and prose accumulates. A third of them are in one nine-hundred-line document written across seven rounds, describing what a future run would show, long after the runs happened. |
| **Commits** | this round |

### A reason that stopped being true, in a comment nobody could have executed

| | |
|---|---|
| **What** | `warehouse_id` defaulted to the empty string, with a comment saying that was so `databricks bundle validate` could still run with no workspace, "which is what CI does". `warehouse_id` is REQUIRED on a dashboard, so the day the dashboard landed the empty default stopped validating. **Measured 6 September 2026: `bundle validate` dies with `dashboard warehouse_id is required`, and so does `bundle run`.** Every command that loads the bundle failed. The only one that worked was `deploy`, because `deploy` is the one command that passes the variable - so the lane could be deployed and could not be launched. |
| **And the reason was never true either** | The `databricks` workflow was `workflow_dispatch` only and **had never been run** when that justification was written: the API reported `total_count: 0` for its entire history. The repository has no `DATABRICKS_HOST`/`DATABRICKS_TOKEN` secrets, so a dispatch would fail at authentication before parsing anything - and `scripts/databricks_run.sh` already said as much in its own comment, "validate is the one step that needs no workspace beyond authentication". The justification cited a behaviour that had never happened once. |
| **How found** | By a deploy that would not deploy. Not by CI, which cannot see this; not by the fast lane, which was not asking; and not by the comment, which asserted the opposite. |
| **Why invisible** | A justification ages differently from the code it justifies. This one was true when it was written - there was no required field behind the variable - and it was made false by a commit that did not touch it, in another file, three days later. Nothing in this repository compares a stated reason against the behaviour it claims, and the sentence read exactly as convincingly after it stopped being true. |
| **The class** | **A written reason is a claim with no test behind it.** The repository already collects declarations that do not govern and comments that assert what does not happen; this is the version where the sentence was *once* correct. Which is worse to read, because the reasoning is sound and the premise has quietly gone. |
| **The trap that was refused** | The obvious repair is a plausible-looking default so that `validate` goes green. That is a check supplying its own input - `validate` would report OK about a warehouse that does not exist - which this repository has already found in a fixture, a document and a spark harness. So the default is a PLACEHOLDER that is deliberately not the shape of a warehouse id (a real one is 16 hex characters), `validate` prints that it ran against it and says what it therefore did not check, and `step_deploy` refuses to deploy with it. |
| **Prevented by** | `tests/fast/test_databricks_bundle.py::test_every_required_field_resolves_to_something_at_deploy_time` - a required field whose value is `${var.X}` is only as present as the default behind it - which is the property the never-run workflow was supposed to provide, moved to the lane that runs on every push with no credentials and no CLI. Falsified against the real defect: with the empty default restored it names both resources. Plus three tests in `tests/fast/test_databricks_catalog_step.py` that execute `validate`, `run` and a refused `deploy` against a stubbed CLI - because this defect exists precisely because nobody ran the path a comment promised. |
| **Found on the way** | The refusal itself did not work the first time. It read the warehouse id with `awk '{print $1}'` over a line of `id state`, and with no warehouse the id is empty - so awk collapsed the leading space and handed back the STATE, and the deploy went through with `--var=warehouse_id=NONE`. A guard defeated by its own input parsing, in a guard added the day before and never executed. `cut -d" " -f1` keeps an empty field empty. |
| **Commits** | this round |
...
| | |
|---|---|
| **What** | Every task in `samegold_close` declared `max_retries: 0`, under a comment reading "Declared, not left to a default". On 5 September 2026 `databricks jobs get` on the DEPLOYED job reported max_retries **not declared** on any of the six tasks, while `timeout_seconds` had survived as 600 and 900. And in run `44869473800771` the task that was failed on purpose ran **twice inside the original run** - attempt 0 failed after 6s, attempt 1 started 57 seconds later and failed after 12s - which is exactly the retry that line was written to prevent. |
| **How found** | Reading the deployed job back after the three runs, which nothing had ever done. The retry itself was visible in the run page as two attempts of one task. |
| **Why invisible** | A retry setting does nothing on every run that succeeds. It is only observable on the day something fails, and on that day the thing you learn is that it was never there. Nothing in this repository compared what the bundle DECLARES against what the workspace RECEIVED - `require_fresh_deployment` compares the deployed commit against HEAD, which is a different question and passed throughout. |
| **The second half, which is the one that decides behaviour** | `max_retries` has a documented default of **0** - "the value 0 means to never retry". So the declaration restated the default, and its arrival or non-arrival cannot explain a retry either way. What retried the task is **serverless auto-optimization**: on by default, described in the UI as "Enable serverless auto-optimization (may include additional retries)", and switched off by the task field `disable_auto_optimization`. The declaration was not just missing. It was the wrong lever, correctly spelled. |
| **The class** | **A declaration that does not govern.** Fourth instance after `development: true` (which describes retry behaviour it does not control), `--full-refresh-all` (announced and not passed), and the bash assignment that set a variable for one command and not the next. First instance where the COMMENT beside it asserts the opposite of what happens - which is worse than silence, because a reader who checks the comment stops looking. |
| **Prevented by** | `disable_auto_optimization: true` on every task that runs compute, beside the `max_retries: 0` that only ever restated a default; `tests/fast/test_databricks_bundle.py::test_no_task_relies_on_max_retries_to_stop_a_serverless_retry` refuses a task that declares one without the other. And `scripts/databricks_run.sh deploy` now READS THE DEPLOYED JOB BACK and prints max_retries, disable_auto_optimization and timeout_seconds per task, warning by name when the one that is not a default fails to arrive - a deploy that succeeds is not a deploy that sent what was written. It reports rather than refuses, because one of the two fields is expected to be absent while the serializer question is open. |
| **Still open** | Whether a bundle drops fields whose value equals their default (a `0` swallowed, a `600` kept) is a hypothesis, not a finding. The falsification costs no compute and is written in `docs/limits.md`: set `max_retries: 2` on one task, deploy, and read the line the deploy step now prints. |
| **Commits** | this round |

### The gate watched the numbers and never once read the sentences

| | |
|---|---|
| **What** | **Sixty-seven statements in the documentation were false**, and the first three were found by a reviewer reading the front page. `docs/milestones.md` said `databricks/resources/` holds `grants.yml`, `jobs.yml` and `volumes.yml`, "and nothing else" - and `dashboards.yml` was added by the same merge that shipped the sentence, so it was **false in the commit that wrote it**. `EXAM_MAP.md` said "nothing has run yet" of a lane that had been running since 3 September with four job run ids committed under `evidence/databricks/`. The `Makefile` said the same of `make databricks`. `PARITY.md` said "this repository has never deployed one" of bundles it deploys with one command. `docs/milestones.md` also called four record sections "never run" two rounds after they were in the record. |
| **How found** | A reviewer read the front page and reported three. A mechanical gate written that afternoon found two more before anything was fixed. Then five review passes read every document under a different lens and a sixth tried to REFUTE each candidate, defaulting to "not false" and looking for a scope or a date that would rescue it: **sixty-seven survived that refutation**. `docs/prose-audit-2026-09-07.md` has the distribution, the clusters, what was fixed and what is still open. |
| **Why invisible** | Every FIGURE in these documents goes through an evidence anchor, and a stale one is a failing test. **The prose around those figures went through nothing.** In a repository whose entire thesis is that a number nobody re-measures is a number that is only true by accident, the sentences had been rotting for days - and the gate that would have caught it was already there, one type of claim away. |
| **The class** | **A gate covers the shape it was built for, and the neighbours inherit its reputation.** The anchors made the numbers trustworthy, and the trust leaked sideways onto the sentences beside them, which nothing checked. It is the same shape as a green tick that reports the linter, one document along. |
| **Prevented by** | `samegold.evidence.prose` checks the three shapes of claim this repository can falsify from its own files: an exhaustive enumeration of a directory (`holds a, b and c, and nothing else` is a closed set, and so is the directory), a claim of the form "has never run" against the committed run records, and a claim that a path does not exist against the filesystem. A sentence that matches one of those shapes and is nevertheless true has to be declared in `EXEMPTIONS` with the measurement behind it, and an exemption that stops matching anything fails the run - so an exception cannot outlive the sentence it was written for. |
| **Born seen failing** | Twice over. It went red on the real repository the moment it existed, on five sentences including two nobody had reported. And `test_the_gate_fires_on_a_sentence_written_tomorrow` hands it three inventions in a document that does not exist, then the same document corrected, so it is known to fire on sentences it was not written against and to go quiet when they are fixed. |
| **What it does not do** | Read intent. It matches three shapes and says so; a false sentence phrased any other way still gets through, and the honest version of this row is that the gate covers what it covers. Which is the finding, one turn later. |
| **Commits** | this round |
...
### The second implementation is linted, type-checked, and executed by nothing

| | |
|---|---|
| **What** | `pipelines/spark-pipeline.yml` and `pipelines/transformations/bronze_silver.py` are the open-source Spark Declarative Pipelines lane. `ruff` lints them (`scripts/preflight.sh`), `mypy` type-checks them (`pyproject.toml` names `pipelines/transformations` explicitly), and `tests/fast/test_contract_documents.py` reads the YAML with `yaml.safe_load` and compares it against the contract. **No Makefile target, no preflight step and no test EXECUTES either file.** The only `spark-pipelines run` in the repository is inside a comment in the YAML's own header. In a project whose thesis is that two implementations agree, one of them had never been started. |
| **And it does not run.** | Measured on 8 September 2026, WSL2 Ubuntu, Temurin 21, pyspark 4.2.0, Delta jars resolved from Maven Central (33 artifacts, already cached from the delta lane). Four attempts, three distinct failures: (1) `spark-pipelines` needs `SPARK_HOME` and a `python` that has pyspark, neither of which a pip install provides - `Could not find valid SPARK_HOME`; (2) with those set, `[SCHEMA_NOT_FOUND] The schema spark_catalog.samegold cannot be found` - the spec declares `database: samegold`, nothing in the repository creates it, and the pipeline does not create it either; (3) creating the schema first, with the same warehouse directory and the same Delta configuration, does not help: the Connect server the CLI starts does not see it. The lane has never produced a table. |
| **The worse half** | The spec's `configuration:` block is **rejected at runtime as a warning**. `spark.sql.extensions` fails with `[CANNOT_MODIFY_STATIC_CONFIG]` and `spark.jars.packages` with `[CANNOT_MODIFY_CONFIG]`, because the block is applied to an ALREADY RUNNING Spark Connect session and a static config can only be set when the server starts. The comment directly above that block says it is not optional - *"Out of the box, open-source SDP writes PARQUET, not Delta: the tables appear under spark-warehouse with no `_delta_log`, and every Delta-specific thing in this project silently has nothing to work with."* The block written to prevent that outcome cannot take effect, and says so through `UserWarning` on stderr rather than by failing. Setting the same keys through `SPARK_CONF_DIR`, where they ARE read at server start, silences the `spark.sql.extensions` warning - which is the measurement that the mechanism, not the value, is what was wrong. |
| **How found** | By asking which target runs it, and finding none. Not by a failure: nothing could fail, because nothing ran. The breakage above was found afterwards, by trying to run it for the first time. |
| **Why invisible** | Every check this lane has is a check on its SHAPE. A linter reads it, a type-checker reads it, and a test parses its YAML - and all three pass on a file that cannot execute. The lane therefore appeared, on every green run, to be as covered as the code beside it. `docs/limits.md` made it worse: its "Written and not executed here" section read **"Nothing, as of 4 September 2026"** and closed with *"An empty list is a claim like any other, and the next thing written and not run belongs in it."* The list had one member and did not list it, in the section written to catch exactly this. |
| **The class** | **A file checked for shape is not a file that runs.** The repository already collects declarations that do not govern; this is the version where every check that exists is real, passes, and is about the wrong property. Second class, from the same measurement: **a setting refused as a warning is indistinguishable from a setting applied** - the pipeline prints two `UserWarning`s and continues, and the only difference a reader would ever see is Parquet where Delta was expected. |
| **Prevented by** | Nothing yet, and that is the honest row. The lane is now IN the `docs/limits.md` list it belonged in, with its date and the three failures above as its reason. Making it run is not a documentation change: the schema has to be created by something, and the static configuration has to move to where the server reads it. Both are in `docs/milestones.md` M11, which is the milestone this lane belongs to and which is not done. |
| **Commits** | this round |

### The harness read its own source, and the extractors accused the figures

| | |
|---|---|
| **What** | Building the three-property gate for the README figures produced two defects in the CHECKING, and both would have shipped as a working gate. **The mutation harness matched against pytest's echo of the test's own source.** pytest prints the failing test's code in the traceback, that code contains the format strings `"NAMES: ..."`, `"EDGES: ..."` and `"NUMBERS: ..."`, and the harness searched the whole run output for those tokens - so every property reported RED for every mutation. **And both extractors first went red about themselves rather than about the figures:** `order_placed` is an event type drawn in free text beside a timeline and was read as a table name, and the label `v0  14 198 046` was read as the number `014198046` because the digit run was unanchored. |
| **How found** | The first by disbelieving a result that was too good: three mutations reddening three properties each is a full grid, and a gate that fires on everything discriminates nothing. The second by reading the failure text instead of the failure count - both messages named the checker's own artefacts, not anything a figure claimed. |
| **Why invisible** | Both wear the costume of a working gate. A harness that reports every property red for every mutation looks, at a glance, like proof that every property is wired up - the table is full, and full reads as pass. And an extractor that goes red on a correct figure has done the one thing this repository asks of a new check: it was **born seen failing**. Acting on that red would have "fixed" two correct diagrams to satisfy a broken reader. |
| **The class** | Two, and they are separate. **A checker fed by its own output** - the harness's input included its own emissions, so it was measuring itself and reporting on the subject. This repository has met the shape once before, in a different costume: the evidence sweep counted the record it had just written as an uncommitted change, and nine records in ten published `tree_dirty: true` because of it. And **born red is half a proof** - a test that fails on arrival has shown it can fail, and has not yet shown it fails for the reason claimed. The two halves are: it goes red when the property is violated, AND the red names the violation rather than the checker. |
| **Prevented by** | The harness reads only lines beginning with `E `, which is the assertion output and not the echoed source, and it asserts the shape of the result rather than its volume: each mutation must redden exactly one property and no other test. A full grid now fails that assertion instead of satisfying it. The extractors were narrowed to what the figure actually claims - NAMES reads labels inside a box, because free text is prose and a box is an assertion that a table exists; NUMBERS anchors the digit run so a `v0` prefix cannot join it. Both were then re-falsified: rename a table, reverse an arrow, change a digit, and each reddens its own property alone. |
| **What it does not do** | Decide whether a red is the right red. Nothing here can: `test_the_digit_normaliser` and the mutation grid raise the cost of a wrong reader, and reading the failure text is still a person's job. The honest statement is that "born seen failing" was being used as a whole proof and is half of one. |
| **Commits** | this round |

---


### The deploy lane went red without a line of the tree changing

| | |
|---|---|
| **What** | `databricks.yml` failed on 22 September 2026 at `bundle validate -t free`, one line after printing the warehouse placeholder, with `Error: error downloading Terraform: unable to verify checksums signature: openpgp: key expired`. Nothing in this repository had changed: the action was pinned by commit SHA, the bundle was byte-identical, the credentials were good. The Databricks CLI downloads a Terraform binary **at run time** - for `validate`, not only for `deploy` - and verifies HashiCorp's OpenPGP signature over its checksum file. That key expired, and every CLI from v0.210.3 to v0.297.1 began failing on command lines that had not moved. [databricks/cli#5022](https://github.com/databricks/cli/issues/5022). |
| **How found** | By a dispatch. The workflow had never run in CI before that afternoon, so this is the first run of a job that had been green on a laptop in early September, and the first thing it found was not about the bundle. |
| **Why invisible** | Nothing in this repository describes the Terraform binary, because this repository never asked for it. The dependency is two levels down - the action installs the CLI, the CLI downloads Terraform - and the only place it appears is in an error message. A green tick on this job on 6 September was a true statement about 6 September and carried no information about 22 September, and no check here could have said so: the failing input arrives over the network at the moment of use. |
| **The class** | **A green that expires.** Not the "written and not executed" shape this repository keeps finding, and not drift either: the tree is correct, the claim is correct, and the world underneath moved. The SHA pin - added on 8 September precisely because `@main` was a moving target on a job that holds a workspace token - did not help and could not have. It fixes WHICH CLI runs, and the thing that changed is what that CLI fetches when it runs. A pin that names the wrong layer is a pin that will be cited as a defence after it has failed to be one. |
| **Prevented by** | Nothing here, and that is the finding rather than a gap to be closed. The pin moved to v0.221.2, which is the patch for this minor line read from the issue's own table rather than inferred from the version number, and `5da7e8b` is the commit that release tagged. What would actually remove the exposure is installing Terraform separately and pointing `DATABRICKS_TF_EXEC_PATH` at it, which makes the CLI skip the download and the signature check - and it is NOT done, for the reason in the next row. |
| **Why `DATABRICKS_TF_VERSION` is not the fix** | Measured in the CLI's own source rather than assumed: `DATABRICKS_TF_VERSION` selects which Terraform to download, and the download still verifies the signature - so pinning it would have changed nothing on the day. The variable that skips the download is `DATABRICKS_TF_EXEC_PATH`, and `findExecPath` then requires the binary's version to equal the one the CLI expects. So the escape hatch is a PAIR of versions that must agree, one of them chosen by a CLI upgrade, with nothing checking they still do until a deploy fails. That is a second unchecked pin, in a repository that already publishes a finding about a justification going stale in a file that never touches it, and it trades a rare external failure for a recurring internal one. `docs/limits.md` carries the exposure instead. |
| **What is not claimed** | That upgrading is a fix for the class. The next expiry, or the next removed artifact, fails the same way, and the only real defences are vendoring the binary or accepting that a deploy lane depends on the network at the moment it runs. This repository accepts it, in writing. |
| **Commits** | this round |
...
| **A file checked for shape is not a file that runs.** Every check can be real, pass, and be about the wrong property. | the OSS declarative lane: linted, type-checked, its YAML parsed, and executed by no target (this round) |
| **A setting refused as a warning is indistinguishable from a setting applied.** | `spark.sql.extensions` in `pipelines/spark-pipeline.yml`, rejected as `CANNOT_MODIFY_STATIC_CONFIG` by the session it is handed to (this round) |
| **A checker fed by its own output.** It measures its own emissions and reports on the subject, and a full grid of results reads as thoroughness. | the mutation harness matching pytest's echo of the test's own source, reddening all three properties for all three mutations (this round); and earlier, the evidence sweep counting the record it had just written as an uncommitted change (`d687813`) |
| **Born red is half a proof.** A test that fails on arrival has shown it CAN fail; that the red names the violation rather than the checker is the other half. | two figure extractors that first went red about themselves - an event type read as a table, a `v0` prefix read into a number - where acting on the red would have "fixed" two correct diagrams (this round) |
...
| **A written reason is a claim with no test behind it, and it ages.** A justification can be true when written and be made false by a commit in another file that never touches it. | the empty `warehouse_id` default justified by what "CI does", on a workflow that had not run at all when the justification was written |
| **A gate covers the shape it was built for, and the neighbours inherit its reputation.** Anchored figures were checked on every push while the sentences around them were checked by nobody. | five false statements in the documents, one of them false in the merge that shipped it (this round) |
...
| **A fix is verified against the case that motivated it and against nothing after it.** The instance it was written for is the only one that exists while it is being written, so the check beside it can only cover that instance. | the late-arrival prefix that separated two populations and not three (this round); and the README figure verified on the ONE edge that had been wrong before, from which the whole topology was pronounced correct - the gate then found three wrong arrows, and this instance was committed by the external reviewer rather than by the repository (this round) |
