...
| **And the reason was never true either** | The `databricks` workflow is `workflow_dispatch` only and **has never been run**: the API reports `total_count: 0` for its entire history. The repository has no `DATABRICKS_HOST`/`DATABRICKS_TOKEN` secrets, so a dispatch would fail at authentication before parsing anything - and `scripts/databricks_run.sh` already said as much in its own comment, "validate is the one step that needs no workspace beyond authentication". The justification cited a behaviour that had never happened once. |
...
| **A written reason is a claim with no test behind it, and it ages.** A justification can be true when written and be made false by a commit in another file that never touches it. | the empty `warehouse_id` default justified by what "CI does", on a workflow that has never run (this round) |
