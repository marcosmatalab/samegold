...
| "Use SQL Alerts to monitor data quality" | `databricks/resources/dashboards.yml` declares one, and it is the answer to a finding rather than a demonstration: it fires on `job_run_status.ok`, not on the job's terminal state, because a run whose close failed reported SUCCESS_WITH_FAILURES. Deployed PAUSED with its cost calculated, for the same quota reason as the job schedule. The OSS-side rule is in `src/samegold/serve/freshness.py` | fast lane, `docs/databricks-run.md` |
| "Build dashboards to visualize data quality and pipeline health" | `databricks/dashboards/samegold_close.lvdash.json`, declared as a bundle resource: the versions of every closed month with what moved between them, the quarantine doors, the last update's expectations, and whether the last run was sound. Its six queries go through Spark's parser and are resolved against the real column names, like the rest of the lane's SQL | needs a workspace |
