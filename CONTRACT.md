...
8. **A refused return is reported, not dropped.** Returns outside the window, for more units
   than were sold, or without an order are counted in `returns_rejected_count` on the month of
   the sale. A record that leaves the pipeline without a counter is the failure nobody detects.
...
| returns operations | `gold.revenue_by_month` at its newest version | < 15 minutes from `event_ts` (declared; the freshness monitor is milestone M16) | may be incomplete; never used for the signed close |
